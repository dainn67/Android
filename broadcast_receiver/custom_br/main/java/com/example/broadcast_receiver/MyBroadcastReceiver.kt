...
class MyBroadcastReceiver : BroadcastReceiver() {
    ...
    private fun isAvailable(context: Context?): Boolean {
        var connectivityManager : ConnectivityManager =  context?.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager ?: return false

        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            var network: Network? = connectivityManager.activeNetwork ?: return false
